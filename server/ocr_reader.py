...
def extract_text(image_bytes: bytes) -> str:
    ...
    try:
        ...
        if response.error and response.error.message:
            ...
            if "BILLING_DISABLED" in msg or "requires billing" in msg:
                raise RuntimeError(
                    "Google Vision API отключён по биллингу. Включите биллинг для проекта и повторите попытку."
                )
            raise RuntimeError(f"Google Vision API error: {msg}")
